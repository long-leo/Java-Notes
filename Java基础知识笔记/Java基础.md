...
- 需要注意的是, 只有当基本类型数据作为类的属性时, 才有默认值, 否则(比如方法内的局部变量)将会是**任意值**
  ...
  ```bash
  所以有一点, 在方法内部即使是static方法是不可以定义static变量的, 因为方法内定义的是局部变量,存在堆栈中,  而static变量是静态变量, 存储区不同.
  ```

![image-20210122144839094](Java基础.assets/image-20210122144839094.png)
...
- > **数组操作方法**
  ...
  - 数组复制
    - System.arraycopy(源数组名称， 源数组复制开始索引， 目标数组名称，目标数组赋值开始索引，长度);  注：与原始定义有差别
  - 数组排序
    - java.util.Arrays.sort(数组名称)  从小到大排序
...
- 调用本类方法
  - 调用构造方法
    ...
    - 构造方法互相调用，应保留出口
  - 调用普通方法
    - 主方法调用主类中的方法时通过给主类中的方法进行static声明
...
> **static定义属性**  （private static String pub = "1245" ；）

- 将属性定义为公共属性，该类所有对象都可以使用，且共用该属性的值
- 此类属性应由类修改，不应由具体对象修改
- static属性可以在没有实例化对象时直接有类名称访问修改
...
- static 不可以定义普通类，抽象类也不可以
- static可以定义内部类，此时该内部类就相当于外部类
  - 不受外部类实例化对象约束
...
- 类中如果没有普通属性，那么再使用这个类时，就没有必要开辟堆内存空间，即实例化对象，此时定义可以考虑定义static方法
- 开发中优先定义非static方法
...
- 构造块  写在类中，**实例化对象**时会调用，可以访问非static属性和static属性，并且先于构造方法执行

  - 把构造方法中都会执行的语句都提取出来放在代码块中
  - 要特别复杂的赋值，用构造代码块

- > **静态块  （类加载时起作用） 代码块用static定义**
  
  - 非主类中
    - 实例化对象时，（**假设属性定义在前，因为属性定义和静态块定义顺序影响执行顺序**）属性定义赋值最先执行，之后是静态块执行，**（如果实例化了对象）**然后是构造块，最后构造方法
    - 静态库**仅执行一次**，无论多少个对象
    - 静态块**可用于static属性的初始化**
    - JDBC的驱动加载
  - 主类中
...
![image-20210130103848616](Java基础.assets/image-20210130103848616.png)
